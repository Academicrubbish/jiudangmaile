...
<template v-else-if="validDeposit"><view class="card" style="margin-top:20px">这一笔，已经记下了。<text class="line-break"> </text><text class="muted">本人确认不代表银行核验，也不代表当前账户余额。</text></view><button class="primary" @click="home">回小店看看 ↗</button><button class="link" :disabled="busy" @click="undo">撤销误记</button></template><button v-else class="primary" @click="home">回到小店 ↗</button>
<button v-if="event.state!=='pending'" class="link" @click="copyStory">复制这件小事，分享给朋友</button></template></view></template>
<script setup>
import {ref,computed} from 'vue';import {onLoad,onShow,onShareAppMessage} from '@dcloudio/uni-app';import TopBar from '../../components/jdml/TopBar.vue';import ItemArt from '../../components/jdml/ItemArt.vue';import {api,money,parseMoney,notify,go,home,isPreview} from '../../services/api';
const event=ref(null),error=ref(''),busy=ref(false),amount=ref('');let eventId='';const expired=computed(()=>event.value?.expiresAt<=Date.now());const validDeposit=computed(()=>event.value?.deposit && !event.value.deposit.revokedAt);const dateLabel=computed(()=>new Date(event.value?.createdAt || Date.now()).toLocaleDateString('zh-CN'));
onLoad(q=>{eventId=q.id || '';});onShow(()=>{if(eventId)load();});
async function load(){error.value='';try{event.value=await api('detail',{id:eventId});if(!amount.value)amount.value=money(event.value.amount);}catch(e){error.value=e.message;}}
async function execute(fn){if(busy.value)return;busy.value=true;try{await fn();}catch(e){notify(e);}finally{busy.value=false;}}
function confirm(){execute(async()=>{const cents=parseMoney(amount.value);event.value=await api('confirm',{id:eventId,amount:cents});});}
